...
def eval_CPN(model, now, img, depth_copy, mask_label, masked_depth, target_t, model_points, diameters, cv2_img):
    ...
    rmse_CPN = rmse(centroid, target_t)
    rmse_surface_depth = rmse(surface_depth, target_t)
    rmse_mean_depth = rmse(mean_depth_xyz, target_t)
    ...
    cv2.circle(cv2_img, (projected_centroid_x, projected_centroid_y), 3, (0,0,255), 3)
    cv2.circle(cv2_img, (projected_target_x, projected_target_y), 3, (255,0,0), 3)
    cv2.circle(cv2_img, (projected_mean_depth_x, projected_mean_depth_y), 3, (0, 255,0), 3)
    # print("rmse_CPN : ", rmse_CPN)
    # print("rmse_surface_depth : ", rmse_surface_depth)
    # print("rmse_mean_depth : ", rmse_mean_depth)

    cpn_error_sum[model] += rmse_CPN
    surface_depth_error_sum[model] += rmse_surface_depth
    mean_depth_error_sum[model] += rmse_mean_depth
    num_of_obj[model] += 1
...
if __name__ == '__main__':
    ...
    if opt.dataset == "ycb":
        for now in tqdm(range(0, len(testlist))):
            ...
            for idx in obj:
                ...
                try:
                    cv2_img = eval_CPN(model, now, img, depth_copy, mask_label, masked_depth, target_t, model_points, diameters, cv2_img)
    ...
    for model in models:
        if surface_depth_error_sum[model] == 0:
            continue
        print(model + " : \t", surface_depth_error_sum[model] / num_of_obj[model])

    avg_error = 0
    num_objs = 0
    for model in models:
        avg_error += surface_depth_error_sum[model]
        num_objs += num_of_obj[model]
    print("Surface depth average error : ", avg_error / num_objs)
    print()
    print("="*40)
    print()
    ...
    for model in models:
        if mean_depth_error_sum[model] == 0:
            continue
        print(model + " : \t", mean_depth_error_sum[model] / num_of_obj[model])

    avg_error = 0
    for model in models:
        avg_error += mean_depth_error_sum[model]
    print("Mean average error : ", avg_error / num_objs)
    ...
    for model in models:
        if cpn_error_sum[model] == 0:
            continue
        print(model + " : \t", cpn_error_sum[model] / num_of_obj[model])
    avg_error = 0
    for model in models:
        avg_error += cpn_error_sum[model]
    print("CenterFindNet average error : ", avg_error / num_objs)